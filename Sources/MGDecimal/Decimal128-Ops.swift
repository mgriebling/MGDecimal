...
extension Decimal128 {
    ...
    static func add(_ x: UInt128, _ y: UInt128, _ rnd_mode: Rounding, _ pfpsf: inout Status) -> UInt128 {
        ...
        var x = x, y = y, res = UInt128.init(upper: 0xbaddbaddbaddbadd, lower: 0xbaddbaddbaddbadd)
        ...
        var P256 = UInt256(), Q256 = UInt256(), R256 = UInt256(), y_nr_bits = 0, x_nr_bits = 0
        ...
        if (((x.hi & MASK_SPECIAL) == MASK_SPECIAL)
            || ((y.hi & MASK_SPECIAL) == MASK_SPECIAL)) {
        ...
        if ((x.hi & 0x6000000000000000) == 0x6000000000000000) {
            ...
            if (C1_hi > 0x0001ed09bead87c0 ||
                (C1_hi == 0x0001ed09bead87c0 && C1_lo > 0x378d8e63ffffffff)) {
                ...
                if (y_exp <= x_exp) {
                    ...
                    if (C2_hi == 0) {    // y_bits is the nr. of bits in C2_lo
                        if (C2_lo >= 0x0020000000000000) {    // y >= 2^53
                            ...
                            y_nr_bits = 32 + Int((((tmp2.bitPattern >> 52)) & 0x7ff) - 0x3ff);
                        } else {    // if y < 2^53
                            tmp2 = Double(C2_lo)    // exact conversion
                            y_nr_bits = Int((((tmp2.bitPattern >> 52)) & 0x7ff) - 0x3ff);
                        }
                    } else {    // C2_hi != 0 => nr. bits = 64 + nr_bits (C2_hi)
                        tmp2 = Double(C2_hi)    // exact conversion
                        y_nr_bits = 64 + Int((((tmp2.bitPattern >> 52)) & 0x7ff) - 0x3ff);
                    }
            ...
            if (x_exp <= y_exp) {
                ...
                if (C1_hi == 0) {    // x_bits is the nr. of bits in C1_lo
                    if (C1_lo >= 0x0020000000000000) {    // x >= 2^53
                        ...
                        x_nr_bits = 32 + Int((((tmp1.bitPattern >> 52)) & 0x7ff) - 0x3ff);
                    } else {    // if x < 2^53
                        tmp1 = Double(C1_lo)    // exact conversion
                        x_nr_bits = Int((((tmp1.bitPattern >> 52)) & 0x7ff) - 0x3ff);
                    }
                } else {    // C1_hi != 0 => nr. bits = 64 + nr_bits (C1_hi)
                    tmp1 = Double(C1_hi)    // exact conversion
                    x_nr_bits = 64 + Int((((tmp1.bitPattern >> 52)) & 0x7ff) - 0x3ff);
                }
                        ...
                        if (ind >= 0) {    // if (x1 >= 1)
                            ...
                            if (ind <= 2) {
                                if (R256.w[1] > 0x8000000000000000 ||
                                    ...
                                    let tmp64A = R256.w[1] - 0x8000000000000000;    // f* - 1/2
                        ...
                        if (x_sign == y_sign) {
                            ...
                            if (C1_hi > 0x0001ed09bead87c0 || (C1_hi == 0x0001ed09bead87c0 && C1_lo >= 0x378d8e6400000000)) {    // C1 >= 10^34
                                ...
                                if (y_exp == EXP_MAX_P1 && (rnd_mode == BID_ROUNDING_TO_NEAREST || rnd_mode == BID_ROUNDING_TIES_AWAY)) {
                                    // overflow for RN
                                    res.hi = x_signi | 0x7800000000000000;    // +/-inf
                                    res.lo = 0x0;
                                    // set the inexact flag
                                    pfpsf.insert(.inexact)
                                    // set the overflow flag
                                ...
                                if (P256.w[1] > 0x8000000000000000 || (P256.w[1] == 0x8000000000000000 && P256.w[0] > 0x0)) {
                                    ...
                                    let tmp64 = P256.w[1] - 0x8000000000000000;    // f* - 1/2
                                    if ((tmp64 > 0x1999999999999999 || (tmp64 == 0x1999999999999999
                                                                        && P256.w[0] >= 0x9999999999999999))) {
                                        // set the inexact flag
                                        pfpsf.insert(.inexact)
                                        is_inexact = true
                            ...
                            if (x_sign == y_sign) {    // addition; could overflow
                                ...
                                if (y_exp == EXP_MAX_P1
                                    && (rnd_mode == BID_ROUNDING_TO_NEAREST
                                        || rnd_mode == BID_ROUNDING_TIES_AWAY)) {
                                    res.hi = 0x7800000000000000 | x_signi    // +/-inf
                                    res.lo = 0x0;
                                    // set the inexact flag
                                    pfpsf.insert(.inexact)
                                    // set the overflow flag
                                ...
                                if ((C1.hi < 0x0000314dc6448d93 || (C1.hi == 0x0000314dc6448d93 && C1.lo < 0x38c15b0a00000000)) ||
                                    (C1.hi == 0x0000314dc6448d93 && C1.lo == 0x38c15b0a00000000 && (is_inexact_gt_midpoint || is_midpoint_lt_even))) {    // C1=10^33
                                    x1 = x1 - 1;    // x1 >= 0
                                    if (x1 >= 0) {
    ...
    static func bid64_fma(_ x: UInt128, _ y: UInt128, _ z: UInt128, _ rnd_mode: Rounding, _ pfpsf: inout Status) -> UInt128 {
        return 0
    }
